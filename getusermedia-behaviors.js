...
GetUserMediaBehaviors.Video = {
  ...
  observers: [
    "_reconfigure(selectedSourceId, resolution, aspectRatio)"
  ],
  ...
  deactivateStream: function(){
    
    if(this.activeStream.active){
      
      console.log(this.activeStream);
      
      this.activeStream.getTracks()[0].stop();
    ...
    MediaStreamTrack.getSources(function(sources){
      ...
      for(var i = 0; i != sources.length; i++){
        ...
        if(sources[i].kind == "video"){
          ...
          newSources.push({id: sources[i].id, label: sources[i].label});
